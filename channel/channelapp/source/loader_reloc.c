...
static bool reloc_dol(entry_point *ep, const u8 *addr, u32 size,
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (check_overlap &&
            ((dolfile->text_start[i] < LD_MIN_ADDR) ||
             ((dolfile->text_start[i] + dolfile->text_size[i] > LD_MAX_ADDR))))
            return false;
    ...
    for (i = 0; i < 11; i++) {
        ...
        if (check_overlap &&
            ((dolfile->data_start[i] < LD_MIN_ADDR) ||
    ...
    for (i = 0; i < ehdr->e_phnum; i++) {
        ...
        if (phdrs[i].p_type != PT_LOAD) {
            ...
            gprintf("PHDR %d 0x%08x [0x%x] -> 0x%08x [0x%x] <", i,
                    phdrs[i].p_offset, phdrs[i].p_filesz, phdrs[i].p_paddr,
                    phdrs[i].p_memsz);
            ...
            if (phdrs[i].p_filesz) {
                ...
                if (phdrs[i].p_flags & PF_X)
                    ICInvalidateRange((void *)phdrs[i].p_paddr,
                                      phdrs[i].p_memsz);
            } else {
                gprintf("-> skip\n");
            }
