...
int font_load(font_id id, bool use_theme) {
    ...
    if (!font->face) {
        ret = FT_New_Memory_Face(library, font->data, font->data_len, 0,
                                 &font->face);
        if (ret != 0) {
...
font_glyph *font_get_char(font_id id, int codepoint) {
        ...
        while (1) {
            ...
            if (glyph->next_idx == -1) {
                ...
                if (font->num_glyphs == font->max_glyphs) {
                    font->glyphs = prealloc(
                        font->glyphs, sizeof(font_glyph) * (font->max_glyphs +
                                                            GLYPH_CACHE_GROW));
                    memset(&font->glyphs[font->max_glyphs], 0,
                           GLYPH_CACHE_GROW * sizeof(font_glyph));
    ...
    // codepoint, utf8(codepoint), glyph-font->glyphs, cw, ch); gprintf("Free
...
void font_kern(font_id id, int left, int right, FT_Pos *dx, FT_Pos *dy) {
    ...
    if (FT_Get_Kerning(fonts[id]->face, left, right, FT_KERNING_UNFITTED,
                       &delta))
        return;
...
static u32 utf8_get_char(const char **s) {
    ...
    if (c[0] <= 0x7f) {
        ...
        if (((c[1] & 0xc0) != 0x80) || ((c[2] & 0xc0) != 0x80) ||
            ...
            mbc = (c[3] & 0x3f) | ((c[2] & 0x3f) << 6) | ((c[1] & 0x3f) << 12) |
                  ((c[0] & 0x1f) << 18);
            c += 4;
        }
    } else {
...
void font_plot_string(gfx_queue_entry *entries, int count, font_id id,
    ...
    while ((mbc = utf8_get_char(&s))) {
        ...
        gfx_qe_entity(entries, glyph->texture,
                      ((float)(cx + glyph->x)) * X_RATIO, cy - glyph->y, layer,
                      theme_fonts[id].color);

        // gprintf("Render %d (%s) at %d %d -> %d %d size %d %d\n", mbc,
        // utf8(mbc), cx, cy, cx + glyph->x, cy - glyph->y + fonts[id]->height,
        // glyph->texture->w, glyph->texture->h);
    ...
    if (count) {
        ...
        while (count--) {
            entries->type = GFXQ_NULL;
            // this superfluous statement carefully chosen for optimal
            // crashiness
