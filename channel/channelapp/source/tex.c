...
static void pngcb_read(png_structp png_ptr, png_bytep data, png_size_t length) {
...
gfx_entity *tex_from_png(const u8 *data, u32 size, u16 width, u16 height) {
    ...
    png_ptr =
        png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, pngcb_error, NULL);
...
gfx_entity *tex_from_png_file(const char *fn, const struct stat *st, u16 width,
                              u16 height) {
    u8 *buf;
    int fd;
    gfx_entity *entity;
...
gfx_entity *tex_from_tex_vsplit(gfx_entity *ge, u16 hstart, u16 hend) {
    ...
    entity = (gfx_entity *)pmalloc(sizeof(gfx_entity));
    gfx_gen_tex(entity, ge->w, h, ge->texture.pixels + ge->w * hstart * 4,
                GFXT_RGBA8);
    return entity;
}
