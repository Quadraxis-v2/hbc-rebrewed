...
static u32 wpad_button_transform(const WPADData *wd, u32 btns) {
    ...
    case WPAD_EXP_CLASSIC:
        ...
        if (btns & WPAD_CLASSIC_BUTTON_Y)
            btns |= WPAD_BUTTON_2;
        if ((btns & WPAD_CLASSIC_BUTTON_FULL_L) ||
            (btns & WPAD_CLASSIC_BUTTON_MINUS))
            btns |= WPAD_BUTTON_MINUS;
        if ((btns & WPAD_CLASSIC_BUTTON_FULL_R) ||
            (btns & WPAD_CLASSIC_BUTTON_PLUS))
...
void controls_scan(u32 *down, u32 *held, u32 *up) {
    ...
    if ((pointer_owner >= 0) &&
        (!wpads[pointer_owner] || !wpads[pointer_owner]->ir.valid)) {
        pointer_owner = -1;
    }
    ...
    if (pointer_owner >= 0) {
        bd |= wpad_button_transform(wpads[pointer_owner],
                                    wpads[pointer_owner]->btns_d);
        bh |= wpad_button_transform(wpads[pointer_owner],
                                    wpads[pointer_owner]->btns_h);
        bu |= wpad_button_transform(wpads[pointer_owner],
                                    wpads[pointer_owner]->btns_u);
    } else {
...
bool controls_ir(s32 *x, s32 *y, f32 *roll) {
    if (pointer_owner >= 0 && wpads[pointer_owner] &&
        wpads[pointer_owner]->ir.valid) {
...
static s32 wpad_sticky(int chan) {
    ...
    if (wpads[chan]) {
        ...
        case WPAD_EXP_NUNCHUK:
            sy = wpads[chan]->exp.nunchuk.js.pos.y -
                 wpads[chan]->exp.nunchuk.js.center.y;
            break;
        case WPAD_EXP_CLASSIC:
            sy = (wpads[chan]->exp.classic.ljs.pos.y -
                  wpads[chan]->exp.classic.ljs.center.y) *
                 4;
            break;
        case WPAD_EXP_GUITARHERO3:
            sy = (wpads[chan]->exp.gh3.js.pos.y -
                  wpads[chan]->exp.gh3.js.center.y) *
                 4;
            break;
        }
    }
    ...
    for (i = 0; i < WPAD_MAX_WIIMOTES; i++) {
        if (on)
            WPAD_SetIdleThresholds(
                i, WPAD_THRESH_DEFAULT_BUTTONS, 10, WPAD_THRESH_DEFAULT_ACCEL,
                WPAD_THRESH_DEFAULT_JOYSTICK, WPAD_THRESH_DEFAULT_BALANCEBOARD,
                WPAD_THRESH_DEFAULT_MOTION_PLUS);
        else
            WPAD_SetIdleThresholds(
                i, WPAD_THRESH_DEFAULT_BUTTONS, WPAD_THRESH_DEFAULT_IR,
                WPAD_THRESH_DEFAULT_ACCEL, WPAD_THRESH_DEFAULT_JOYSTICK,
