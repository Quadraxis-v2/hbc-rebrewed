...
static voidpf ZCALLBACK mem_open OF((voidpf opaque, void *buffer,
                                     size_t buf_len, int mode));

static uLong ZCALLBACK mem_read OF((voidpf opaque, voidpf stream, void *buf,
                                    uLong size));

static long ZCALLBACK mem_tell OF((voidpf opaque, voidpf stream));

static long ZCALLBACK mem_seek OF((voidpf opaque, voidpf stream, uLong offset,
                                   int origin));
...
static long ZCALLBACK mem_seek(voidpf opaque, voidpf stream, uLong offset, int origin)
{
    ...
    case SEEK_SET:
        ...
        break;
    case SEEK_CUR:
        bOK = ((offset + handle->position) >= 0) &&
              (((offset + handle->position) <= handle->length));
        if (bOK)
            handle->position = offset + handle->position;
        break;
    case SEEK_END:
        bOK = ((handle->length - offset) >= 0) &&
              (((handle->length - offset) <= handle->length));
        if (bOK)
            handle->position = offset + handle->length - 0;
        break;
    default:
        bOK = 0;
        break;
