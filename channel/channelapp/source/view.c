...
void view_theme_reinit(void) {
    gfx_qe_entity(&entry_bg, theme_gfx[THEME_BACKGROUND], 0, 0, -3,
                  COL_DEFAULT);
    ...
    gfx_qe_entity(&entry_throbber, theme_gfx[THEME_THROBBER],
                  ...
                  (view_height - theme_gfx[THEME_THROBBER]->h) / 2, 0,
                  COL_DEFAULT);
}
...
static void view_push_view(const view *v, u32 subview_alpha) {
    ...
    if (v->sub_view) {
        ...
        gfx_gen_gradient(&gradient_subview, view_width + 10, view_height + 10,
...
void view_enable_cursor(bool enable) { cursor_enabled = enable; }
...
void view_throbber_tickle(void) { entry_throbber.entity.rad -= 0.1f; }
