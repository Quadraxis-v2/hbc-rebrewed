...
bool update_busy(bool *update_available) {
    ...
    if (us == UPDATE_FETCH_META) {
        ...
        *update_available = false;
        ...
        res =
            LWP_CreateThread(&update_thread, update_func, &ta_ua, update_stack,
                             UPDATE_THREAD_STACKSIZE, UPDATE_THREAD_PRIO);
...
bool update_execute(view *sub_view, entry_point *ep) {
    ...
    snprintf(text, UPDATE_MSG_SIZE,
             _("An update to the Homebrew Channel "
    ...
    b = loader_reloc(ep, data, length, "installer.elf\0-updatehbc\0\0", 26,
                     true);
