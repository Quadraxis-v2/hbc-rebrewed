...
bool loader_gecko_initialized(void) { return ta_gecko.running; }
...
typedef enum { LDC_FILE = 0, LDC_GECKO, LDC_TCP } ld_load_cmd;
...
void loader_load(loader_result *result, view *sub_view, app_entry *entry) {
    ...
    if (ta.cmd == LDC_FILE) {
        ...
        if (entry->meta && entry->meta->args) {
            result->args_len++;
            memcpy(result->args + result->args_len, entry->meta->args,
                   entry->meta->argslen);
    ...
    if (manage_is_zip(ta.data)) {
        ...
        if (result->type == LT_UNKNOWN) {
            ...
            ta.data = NULL;
            ta.data_len = 0;
            show_message(sub_view, DLGMT_ERROR, DLGB_OK, text_err_invalid_zip,
                         0);
        }
...
bool loader_load_executable(entry_point *ep, loader_result *result,
                            view *sub_view) {
    bool res = loader_reloc(ep, result->data, result->data_len, result->args,
                            result->args_len, true);
...
bool loader_handle_zip_app(loader_result *result, view *sub_view) {
    ...
    text_extract_zip =
        _("Extract the received ZIP file?\n%s of free space are required.");
    ...
    if (show_message(sub_view, DLGMT_CONFIRM, DLGB_YESNO, buf, 0) != 0) {
        blob_free(result->data);
        return false;
    }

    if (!manage_run(sub_view, result->dirname, result->data, result->data_len,
                    result->bytes)) {
