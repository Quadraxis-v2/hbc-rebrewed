...
static device_t devices[DEVICE_COUNT] = {{"sd", &__io_wiisd, false},
...
app_sort app_entry_get_sort(void) { return current_sort; }

void app_entry_set_sort(app_sort sort) {
    // TODO: Cleanup here - if we are comparing one thing we shouldn't need to
    // modify fields that are most of the time set to false anyways
    switch (sort) {
    case APP_SORT_DATE:
        cmp_author = false;
        cmp_descending = true;
...
static bool _mount(int index) {
    devices[index].last_status =
        fatMountSimple(devices[index].name, devices[index].device);
...
bool app_entry_get_filename(char *buf, const app_entry *app) {
    ...
    sprintf(buf, "%s:%s/%s/", devices[device_active].name, app_path,
            app->dirname);
