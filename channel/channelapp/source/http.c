...
typedef enum { HTTPCMD_IDLE = 0, HTTPCMD_EXIT, HTTPCMD_FETCH } http_cmd;
...
static void *http_func(void *arg) {
    ...
    while (true) {
        ...
        r += sprintf(r,
                     "User-Agent: TheHomebrewChannel/%s Wii/%08lx"
                     " (%u; %u; %s-%s)\r\n",
                     CHANNEL_VERSION_STR, ng_id, ms_id, ta->sysmenu_version,
                     ta->region, ta->area);
...
bool http_ready(void) { return ta_http.running && ta_http.done; }
